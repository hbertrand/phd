...
If we define the \textit{domain} as the data ant its probability distribution, then transfer learning can be divided along two axis. How are the source and target domains related, and how are the source and target tasks related ? 

TODO
2x2 quadrant same/same = traditional ml
diff/same = transductive transfer
same/diff = inductive transfer
diff/diff = unsupervised transfer

While we briefly present all three kind of transfer learning below, this work focuses on transductive transfer learning, i.e. when the task stays the same, but the domain changes.

Survey~\textcite{pan2010TNDE}

Reasons for using transfer learning : lack of target data, cost of training from scratch
...
Fine tuning (\textcite{shin2016}) is the most commonly used technique to perform transfer learning for deep networks: a network trained on a source dataset is retrained on a target dataset with a lower learning rate. No modifications to the architecture are done, making it simple to implement. Unfortunately performance on the source dataset is no longer guaranteed which makes it insufficient for our application.
